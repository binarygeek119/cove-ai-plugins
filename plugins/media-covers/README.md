...
Install **AI Provider** 0.2.2+ first (`com.binarygeek119.ai-provider`). Set the API key, URL, and image model there (Venice default is `qwen-image-2`).
...
6. If Venice flags a **content violation**, the image is not kept. The job retries with likely trigger content left out (story, one half of the tags then the other, all tags, then a softened title). If that still fails, it makes a **plain stylized title poster** with no people.
